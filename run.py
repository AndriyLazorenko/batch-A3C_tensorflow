...
if params['rom'] == 'toy_way':
    env = env_way.env_way(params)
else:
    env = env_atari.env_atari(params)
# log-uniform learning rate setting (reference : https://github.com/miyosuda/async_deep_reinforce ) params['lr_init']
#  = np.exp(np.log(params['lr_loguniform_low']) * (1-params['lr_loguniform_seed']) + np.log(params[
# 'lr_loguniform_high']) * params['lr_loguniform_seed'])
img = env.reset()
...
for i in range(params['num_workers']):
    ...
    _thread.start_new_thread(workers[i].run_worker, (i,))
...
def do_training():
    ...
    if params['LSTM']:
        ...
        # _,e,gf,loss_v,loss_p,entropy = sess.run([net['train'],net['loss_total'],net['global_frame'],net['loss_v'],
    # net['loss_p'],net['entropy']],tfd)
    _, e, gf, loss_v, loss_p, entropy, grad = sess.run(
        [net['train'], net['loss_total'], net['global_frame'], net['loss_v'], net['loss_p'], net['entropy'],
         net['grad_norm']], tfd)
...
while gf < params['max_T']:
    ...
    if gf > last_eval_frame + params['eval_interval'] or params['eval_mode']:
        ...
        while num_epi < params['eval_duration']:
            per[0, :, :, 0:params['img_c'] * (params['history'] - 1)] = per[0, :, :,
                                                                        params['img_c']:params['img_c'] * params[
                                                                            'history']].copy()
            ...
            if params['LSTM']:
                fd[net['LSTM_h_ph']] = LSTM_h;
                fd[net['LSTM_c_ph']] = LSTM_c;
                fd[net['unroll']] = np.array([1])
        ...
        print('Evaluation Running Time : ' + str(time.time() - eval_start_time) + ' (# of frames learned: ' + str(
            gf) + ' / ' + str(params['max_T']) + ')')
