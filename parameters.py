...
def load_params():
    params = {
        # Meta
        'log_name': 'A3C',
    # the tensorboard event files will be saved in the folder 'params[log_path]+params[log_name]'
        'log_path': 'logs/',
        ...
        'num_actions': 0,
    # number of possible actions (automatically detected if the environment class has 'action_space.n')
        'img_w': 84,
        'img_h': 84,
        'img_c': 1,
        ...
        'convs_size': [3, 3, 3, 3],
    # filter sizes in conv layers. len(params['convs_size']) is the number of convolutional layers
        'convs_filter': [32, 32, 32, 32],  # output channel in conv layers
        'convs_stride': [2, 2, 2, 2],  # stride in conv layers
        'dim_fc': [],
    # dimensions of fc layers. len(params['dim_fc']) is the number of fc layers. it does not include LSTM layer.
        ...
        'score_display_interval': 5,
    # episodes. display 0th thread's mean score after every 'score_display_interval' episodes
        'save_interval': 1000000,  # frames. save the check point in every 'save_interval' frames
        'eval_interval': 30 * 4 * (10 ** 6) - 2,
		# frames. Pause the learning and do the evaluation in every 'eval_interval' frames
    ...
    # epsilon will be annealed lineary and it will be 'eps_min' after this frame.
