...
def conv_layer(layer_name, inputs, filters, size, stride, activation='lrelu'):
    ...
    weight = tf.Variable(tf.random_uniform([size, size, int(channels), filters], minval=-bound, maxval=bound),
                         name=layer_name + '/w')
    ...
    print(
        '    Layer  "%s" : Type = Conv, Size = %d * %d, Stride = %d, Filters = %d, Input channels = %d, activation = %s' % (
        layer_name, size, size, stride, filters, int(channels), activation))

    if activation == 'linear':
        return conv_biased
    elif activation == 'relu':
        return tf.nn.relu(conv_biased, name=layer_name + '/o')
    elif activation == 'lrelu':
        return tf.maximum(0.1 * conv_biased, conv_biased, name=layer_name + '/o')
    elif activation == 'elu':
        return tf.nn.elu(conv_biased, name=layer_name + '/o')
    elif activation == 'tanh':
        return tf.nn.tanh(conv_biased, name=layer_name + '/o')
    elif activation == 'sigmoid':
        return tf.nn.sigmoid(conv_biased, name=layer_name + '/o')
    elif activation == 'softmax':
        return tf.nn.softmax(conv_biased, name=layer_name + '/o')
    elif activation == 'softplus':
        return tf.nn.softplus(conv_biased, name=layer_name + '/o')
    else:
        raise ValueError
...
def fc_layer(layer_name, inputs, hiddens, activation='lrelu'):
    ...
    print('    Layer  "%s" : Type = Full, Hidden = %d, Input dimension = %d,  Activation = %s' % (
    layer_name, hiddens, int(dim), activation))
    ip = tf.add(tf.matmul(inputs, weight), biases, name=layer_name + '/ip')
    if activation == 'linear':
        return ip
    elif activation == 'relu':
        return tf.nn.relu(ip, name=layer_name + '/o')
    elif activation == 'elu':
        return tf.nn.elu(ip, name=layer_name + '/o')
    elif activation == 'lrelu':
        return tf.maximum(0.1 * ip, ip, name=layer_name + '/o')
    elif activation == 'tanh':
        return tf.nn.tanh(ip, name=layer_name + '/o')
    elif activation == 'sigmoid':
        return tf.nn.sigmoid(ip, name=layer_name + '/o')
    elif activation == 'softmax':
        return tf.nn.softmax(ip, name=layer_name + '/o')
    elif activation == 'softplus':
        return tf.nn.softplus(ip, name=layer_name + '/o')
    else:
        raise ValueError
...
def deconv_layer(layer_name, inputs, w_size, out_h, out_w, out_c, stride, activation='lrelu'):
    ...
    weight = tf.Variable(tf.random_uniform([w_size, w_size, out_c, int(channels)], minval=-bound, maxval=bound),
                         name=layer_name + '/w')
    biases = tf.Variable(tf.random_uniform([out_c], minval=-bound, maxval=bound), name=layer_name + '/b')

    deconv = tf.nn.conv2d_transpose(inputs, weight, [tf.shape(inputs)[0], out_h, out_w, out_c],
                                    strides=[1, stride, stride, 1], padding='SAME', name=layer_name + '/dc')
    deconv_biased = tf.add(deconv, biases, name=layer_name + '/db')

    print(
        '    Layer  "%s" : Type = Deconv, W_Size = %d * %d, Out_Size = %d * %d, Out_channels = %d, Stride = %d, activation = %s' % (
        layer_name, w_size, w_size, out_h, out_w, out_c, stride, activation))

    if activation == 'linear':
        return deconv_biased
    elif activation == 'relu':
        return tf.nn.relu(deconv_biased, name=layer_name + '/o')
    elif activation == 'lrelu':
        return tf.maximum(0.1 * deconv_biased, deconv_biased, name=layer_name + '/o')
    elif activation == 'elu':
        return tf.nn.elu(deconv_biased, name=layer_name + '/o')
    elif activation == 'tanh':
        return tf.nn.tanh(deconv_biased, name=layer_name + '/o')
    elif activation == 'sigmoid':
        return tf.nn.sigmoid(deconv_biased, name=layer_name + '/o')
    elif activation == 'softmax':
        return tf.nn.softmax(deconv_biased, name=layer_name + '/o')
    elif activation == 'softplus':
        return tf.nn.softplus(deconv_biased, name=layer_name + '/o')
    else:
        raise ValueError
