...
class env_way:
    def __init__(self, params):
        ...
        self.poss = np.concatenate((np.repeat(temp, params['field_size']).reshape((-1, 1)),
                                    np.tile(temp, params['field_size']).reshape((-1, 1))), axis=1)
    ...
    def reset(self):
        ...
        self.img = np.zeros(
            (self.p['field_size'] * self.p['pixel_per_grid'], self.p['field_size'] * self.p['pixel_per_grid'], 3))
        self.deploy()
        self.render()
        self.term = 0
    ...
    def render(self):  # agent : green, waypoints : red(25*idx)
        ...
        for i in range(self.p['field_size']):
            for j in range(self.p['field_size']):
                self.img[i * self.p['pixel_per_grid']:(i + 1) * self.p['pixel_per_grid'],
                j * self.p['pixel_per_grid']:(j + 1) * self.p['pixel_per_grid'], :] = self.field[i, j, :]
