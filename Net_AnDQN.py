...
def build(params, net_name, device="/gpu:0"):
    ...
    with tf.variable_scope(net_name) as vs:
        ...
        with tf.variable_scope('main_net') as vs_main:
            ...
            with tf.variable_scope('fea') as vs_fea:
                ...
                for i in range(len(params['convs_size'])):
                    convs.append(
                        tu.conv_layer('conv' + str(i), inputs, params['convs_filter'][i], params['convs_size'][i],
        ...
        with tf.variable_scope('target_net') as vs_target:
            ...
            with tf.variable_scope('common') as vs_c:
                ...
                for i in range(len(params['dim_fc'])):
                    target_fcs.append(
                        tu.fc_layer('fc' + str(i), inputs, hiddens=params['dim_fc'][i], activation=activations))
                    inputs = target_fcs[-1]
                ...
                if params['LSTM']:
                    ...
                    target_LSTM_output, target_LSTM_state = tf.nn.dynamic_rnn(target_cells, inputs,
                                                                              initial_state=target_state_tuple,
                                                                              sequence_length=unroll)
    ...
    if params['LSTM']: output['unroll'] = unroll; output['LSTM_h_ph'] = LSTM_h_ph; output['LSTM_c_ph'] = LSTM_c_ph;
    output['LSTM_state'] = fc2_state
